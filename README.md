...
void loop() {
  if (Eyes::closerThanCm(15)) {
    Face::show(Icon::Surprised);
    Legs::walkBackward(2);
  } else {
    Face::show(Icon::Happy);
    Legs::walkForward(1);
  }
}
```
...
| Show a heart | `putMouth(13)` or `putMouth(heart)` | `Face::show(Icon::Heart)` |
...
```
Facades   Face::show(Icon::Heart)          ← expressive, beginner-friendly
...
```cpp
#include <OttoFlow.h>
void setup() { OttoFlow::start(); Face::show(Icon::Heart); }
void loop()  {}
```
